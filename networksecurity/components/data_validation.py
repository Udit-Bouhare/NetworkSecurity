        ...
        try: 
            ...
            if not status: 
                raise NetworkSecurityException(
                    "Train dataframe contains non-numeric columns where numeric excepted"
                )
            
            status = self.validate_numerical_columns(dataframe=test_dataframe)
            if not status: 
                raise NetworkSecurityException(
                    "Test dataframe contains non-numeric columns where numeric excepted"
                )
            

            # lets check datadrify 
